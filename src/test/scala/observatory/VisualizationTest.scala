...
class VisualizationTest extends FunSuite with Checkers {
  ...
  test("'inverseDistanceWeighting' in a range of temperatures if the point exists in the range") {
    val p = Location(37.35, -78.433)
    val range = Seq(
      (p, 27.3),
      (Location(37.358, -78.438), 1.0)
    )
    val actual = Visualization.estimate(p, range)
    val expected = 27.3

    assertResult(expected)(actual)
  }

  test("'inverseDistanceWeighting' in a range if the point doesn't exist in the range") {
